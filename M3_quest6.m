g=inline('((t+2).^(2)).*(t>=-3 & t<-1)+(t.^(2)).*(t>=-1 & t<1)+((t-2).^(2)).*(t>=1 & t<3)','t');
x=inline('t.*(t>=0 & t<=1.5)','t');
%convolution function 
dtau = .005; %defines an incremental value for the convolution
tau= -7:dtau:7; %defines the integrating ranges for the convolution.
ti= 0; %initialising the index counter, i.e. pointer to the solution to DE in y
tvec = -7:.1:7; %defines time range for the output presentation
